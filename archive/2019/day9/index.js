...
let originalProgram = input.toString().split(",").map(BigInt);
...
function executeProgram(program, input) {
  ...
  while (memory[program.pointer] !== 99n) {
    let [instruction, _, arg1Mode, arg2Mode, arg3Mode] = parseOpCode(
      memory[program.pointer]
    );
    ...
    let param1 = getParamValue(
      program,
      parseInt(instruction),
      arg1,
      parseInt(arg1Mode)
    );
    let param2 = getParamValue(
      program,
      parseInt(instruction),
      arg2,
      parseInt(arg2Mode)
...
function parseOpCode(opcode) {
  return opcode.toString().padStart(5, 0).split("").reverse();
}
